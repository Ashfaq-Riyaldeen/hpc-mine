...
Platform: Linux $\cdot$ GCC 15.2 $\cdot$ OpenMPI $\cdot$ 32 logical CPU cores\\[2em]
May 2026}
...
\textbf{Phase 5 (MAE):}  The \code{reduction(+:err)} clause gives each thread a
private accumulator; OpenMP sums all copies at the barrier.
...
All parallel implementations are validated with \textbf{Mean Absolute Error
(MAE)} and a \textbf{similarity-matrix checksum}:
\[
  \text{MAE} = \frac{1}{|T|} \sum_{(u,i) \in T} |\hat{r}_{ui} - r_{ui}|
  \qquad (|T| = 29{,}866)
\]

\textbf{Why MAE and not RMSE?}  MAE treats all errors equally on the bounded
1--5 rating scale.  RMSE would disproportionately penalise occasional large
errors.

\begin{table}[H]
\centering
\begin{tabular}{L{3cm} C{2cm} C{2cm} C{4.5cm} R{3cm}}
\toprule
\textbf{Version} & \textbf{Workers} & \textbf{MAE} &
...
Hybrid 2$\times$4 & 8 & 1.2574 & YES $(\Delta=0.0000)$ & 942.387323 \\
Hybrid 4$\times$2 & 8 & 1.2574 & YES $(\Delta=0.0000)$ & 942.387323 \\
Hybrid 8$\times$1 & 8 & 1.2574 & YES $(\Delta=0.0000)$ & 942.387323 \\
Hybrid 1$\times$8 & 8 & 1.2574 & YES $(\Delta=0.0000)$ & 942.387323 \\
CUDA           & GPU & N/A    & N/A (no GPU)            & N/A \\
\bottomrule
\end{tabular}
\caption{MAE correctness. All 17 CPU runs produce identical MAE and checksum.}
\end{table}

All 17 CPU-based runs produce MAE = \textbf{1.2574} and checksum =
\textbf{942.387323} --- confirming zero race conditions and correct
partitioning across every implementation.
