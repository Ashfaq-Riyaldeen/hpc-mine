...
# ─────────────────────────────────────────────────────────────────────────────
# ── DATA — parsed from the real benchmark run in results/ ────────────────────
# Single source of truth: results/speedup_table.csv + results/mae_comparison.txt.
# To refresh: re-run  bash results/run_benchmarks.sh , then re-run this script.
# No numbers are hardcoded here, so the charts can never drift from the data.
# ─────────────────────────────────────────────────────────────────────────────

# Problem size used in the benchmark (matches results/timing_raw.txt header)
N_USERS = 1000
N_ITEMS = 1000

RESULTS_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "results")
CSV_PATH    = os.path.join(RESULTS_DIR, "speedup_table.csv")
MAE_PATH    = os.path.join(RESULTS_DIR, "mae_comparison.txt")

def _load_csv(path):
    """Return {(version, workers_str): {sim, pred, tot}} from speedup_table.csv."""
    rows = {}
    with open(path) as fh:
        ...
        for line in fh:
            ...
            if not line:
                continue
            p = line.split(",")
            rows[(p[0], p[1])] = {"sim": float(p[2]),
                                  "pred": float(p[3]),
                                  "tot": float(p[4])}
    return rows

def _serial_mae(path):
    """Serial MAE from mae_comparison.txt (every version matches it by design)."""
    with open(path) as fh:
        for line in fh:
...
_R   = _load_csv(CSV_PATH)
_MAE = _serial_mae(MAE_PATH)   # identical across all versions (correctness check)

WORKERS = [1, 2, 4, 8]

def _series(prefix, workers):
    sim  = [_R[(prefix, str(w))]["sim"]  for w in workers]
    pred = [_R[(prefix, str(w))]["pred"] for w in workers]
    return sim, pred

# ── Serial baseline ───────────────────────────────────────────────────────────
SERIAL_SIM  = _R[("Serial", "1")]["sim"]
SERIAL_PRED = _R[("Serial", "1")]["pred"]
SERIAL_TOT  = _R[("Serial", "1")]["tot"]
SERIAL_MAE  = _MAE

# ── OpenMP / Pthreads / MPI at 1, 2, 4, 8 workers ────────────────────────────
OPENMP_SIM,   OPENMP_PRED   = _series("OpenMP",   WORKERS)
PTHREADS_SIM, PTHREADS_PRED = _series("Pthreads", WORKERS)
MPI_SIM,      MPI_PRED      = _series("MPI",       WORKERS)
OPENMP_MAE   = [_MAE] * 4
PTHREADS_MAE = [_MAE] * 4
MPI_MAE      = [_MAE] * 4

# ── CUDA (single GPU run; CSV worker label is "GPU") ─────────────────────────
if ("CUDA", "GPU") in _R:
    CUDA_SIM  = _R[("CUDA", "GPU")]["sim"]
    CUDA_PRED = _R[("CUDA", "GPU")]["pred"]
    CUDA_MAE  = _MAE
else:
...
# ─────────────────────────────────────────────────────────────────────────────
def chart_amdahl():
    # Effective serial fraction fitted from the measured OpenMP-8T speedup.
    # Solving Amdahl S(8) = 1/(f + (1-f)/8) for f gives the effective serial
    # fraction that absorbs hyperthreading and memory-bandwidth overhead
    # (larger than the ~0.3% pure-algorithmic serial fraction).
    f_serial = (1.0 / OMP_SU[-1] - 1.0 / 8) / (1 - 1.0 / 8)
