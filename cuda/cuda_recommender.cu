 ...
 *    sm_60 = Pascal, sm_70 = Volta, sm_75 = Turing,
 *    sm_80 = Ampere, sm_86 = RTX 30xx, sm_89 = RTX 40xx)
 */
...
/* ── Phase 1: Data generation (serial, identical seed to all versions) ──────  */
...
/* Phase 2 – user means: one thread per user */
...
/* Phase 3 – Pearson similarity: 2D grid, one thread per (u,v) pair.
 * Only threads with u < v do work; each writes both SIM(u,v) and SIM(v,u).
 * The diagonal is set by the u==v branch. */
                                   ...
                                   float *sim_matrix, int N_USERS, int N_ITEMS)
{
    ...
    if (u >  v) return;   /* upper-triangle only; mirror written by u<v thread */
...
/* Phase 4 – predictions: 2D grid, one thread per (user, item) pair.
 *
 * Top-K selection is done with a fixed-size in-register buffer.
 * TOP_K=20 means three 20-element float arrays per thread (~240 B each thread).
 * On modern GPUs these live in registers / local memory and fit well within
 * per-SM limits at typical occupancy. */
                                    ...
                                    int N_USERS, int N_ITEMS)
{
    ...
    /* maintain top-K neighbours in fixed-size register arrays */
    float top_sim[TOP_K];
    float top_rat[TOP_K];
    float top_mu [TOP_K];
    int   top_cnt = 0;
...
int main(int argc, char *argv[])
{
    ...
    /* Phase 1 – data generation (host, serial) */
    ...
    /* ── Phase 2: User means ──────────────────────────────────────────────── */
    {
        ...
        kernel_user_means<<<grid, block>>>(d_ratings, d_user_mean, N_USERS, N_ITEMS);
        ...
        CUDA_CHECK(cudaMemcpy(h_user_mean, d_user_mean,
                              N_USERS * sizeof(float), cudaMemcpyDeviceToHost));
    }
